...
export default function UserProfilePage() {
  ...
  const activeUser = {
    ...
    imageUrl: user?.imageUrl || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=1974&auto=format&fit=crop",
  };
            ...
            <Reveal className="bg-zinc-900 rounded-[3rem] p-10 text-white space-y-8 relative overflow-hidden group">
               ...
               <div className="relative z-10 space-y-6 text-center">
                  <div className="relative w-32 h-32 mx-auto rounded-full overflow-hidden border-4 border-primary shadow-2xl">
                     <Image src={activeUser.imageUrl} alt={t('profile')} fill className="object-cover" />
                  </div>
                  <div className="space-y-1">
                     <h2 className="text-3xl font-headline font-black italic tracking-tighter">{activeUser.fullName}</h2>
                     <p className="text-[10px] font-black uppercase text-primary tracking-[0.4em]">{t('eliteMember')}</p>
                  </div>
               </div>
               ...
               <div className="pt-8 border-t border-white/5 space-y-6 relative z-10">
                  ...
                  <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-zinc-500">
                     <span>{t('eliteStatus')}</span>
                     <span className="text-primary font-black">{t('active')}</span>
                  </div>
               </div>
            </Reveal>

            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-xl space-y-8">
               <h3 className="font-headline font-black italic text-xl">{t('quickActions')}</h3>
               <div className="space-y-4">
                  <button className="w-full text-left p-4 rounded-2xl hover:bg-slate-50 transition-all flex items-center justify-between group">
                     <span className="font-body italic text-sm text-slate-500 group-hover:text-zinc-900 transition-colors">{t('editDetails')}</span>
                     <span className="material-symbols-outlined text-sm text-slate-300">chevron_right</span>
                  </button>
                  <button className="w-full text-left p-4 rounded-2xl hover:bg-slate-50 transition-all flex items-center justify-between group">
                     <span className="font-body italic text-sm text-slate-500 group-hover:text-zinc-900 transition-colors">{t('manageReservations')}</span>
                     <span className="material-symbols-outlined text-sm text-slate-300">chevron_right</span>
                  </button>
               </div>
            </div>
          ...
          <div className="lg:col-span-8 space-y-12">
            <Reveal className="space-y-8">
               <header className="space-y-4">
                  <h1 className="text-6xl md:text-8xl font-headline font-black italic tracking-tighter leading-none">Your <br /><span className="text-primary">{t('yourJournal')}</span></h1>
               ...
               <div className="grid grid-cols-1 gap-6">
                  <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl flex items-center gap-8">
                     ...
                     <div className="flex-grow">
                        <h4 className="font-headline font-black text-2xl italic tracking-tight">{t('upcoming')}: Zuma Dubai</h4>
                        <p className="text-sm text-slate-400 font-body italic uppercase tracking-widest mt-1">Tomorrow, 20:30 • 4 Guests</p>
                     </div>
                     <button className="bg-zinc-900 text-white px-8 py-4 rounded-xl font-headline font-black italic text-xs hover:bg-primary transition-all">{t('manage')}</button>
                  </div>
                  ...
                  <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl flex items-center gap-8 opacity-50">
                     ...
                     <div className="flex-grow">
                        <h4 className="font-headline font-black text-2xl italic tracking-tight text-zinc-400">{t('past')}: Al Mahara</h4>
            ...
            <Reveal className="bg-zinc-900 rounded-[4rem] p-16 text-white space-y-10 relative overflow-hidden">
               ...
               <div className="space-y-4 max-w-xl relative z-10">
                  <span className="text-primary font-body text-[10px] font-black uppercase tracking-[0.4em] block">{t('rewardsLabel')}</span>
                  <h3 className="text-5xl font-headline font-black italic tracking-tighter leading-none">{t('benefitsTitle')}</h3>
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-8 relative z-10 pt-8">
                  {[
                    { label: t('points'), val: "2.4k" },
                    { label: t('priority'), val: "Tier 1" },
                    { label: t('visits'), val: "34" },
                    { label: t('badges'), val: "8" }
                  ].map(stat => (
