...
export default function ChefDashboard() {
  ...
  const stats = [
    { label: t('profileImpressions'), value: "8.4k", change: "+14%", icon: "visibility" },
    { label: t('restaurantTraffic'), value: "24.2k", change: "+8%", icon: "restaurant" },
    { label: t('curationPulse'), value: "4.9", change: t('stable'), icon: "bolt" },
    { label: t('mediaAssets'), value: "142", change: "+4", icon: "photo_library" },
  ];

  return (
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <header className="pb-12 border-b border-white/5">
        <span className="text-secondary font-body text-[10px] font-black uppercase tracking-[0.4em] block mb-4">{t('culinaryCommand')}</span>
        <h1 className="text-6xl md:text-8xl font-headline font-black italic tracking-tighter">
          The <span className="text-primary">{t('thePortfolio')}</span>
        </h1>
      </header>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-8 p-12 bg-white/5 rounded-[3rem] border border-white/5 space-y-12">
          <h3 className="text-3xl font-headline font-black italic">{t('recentMentions')}</h3>
            ...
            {[1, 2].map(i => (
              <div key={i} className="flex gap-8 items-center p-8 bg-white/2 rounded-2xl border border-white/5 group hover:border-primary/20 transition-all">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-widest text-zinc-600 mt-2">{t('verifiedGuest')} · Al-Maha Crystal Lounge</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-4 p-12 bg-white/5 rounded-[3rem] border border-white/5 space-y-8">
          <h3 className="text-3xl font-headline font-black italic">{t('liveVenues')}</h3>
