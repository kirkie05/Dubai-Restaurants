...
export default function ChefRestaurants({ params }: { params: { slug: string } }) {
  const chefName = params.slug.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(' ');
          ...
          <div className="flex flex-col lg:flex-row items-end justify-between gap-16 pb-16 border-b border-slate-100">
             <div className="space-y-6 max-w-4xl">
                ...
                <h1 className="text-6xl md:text-9xl font-headline font-black italic tracking-tighter leading-[0.85] text-on-surface">
                   The <br /><span className="text-zinc-400">Library.</span>
                </h1>
             ...
             <Link href={`/chefs/${params.slug}`} className="text-primary font-body text-[10px] font-black uppercase tracking-[0.3em] pb-1 border-b border-primary/20 hover:border-primary transition-all">Back to Profile</Link>
          </div>
        </header>
